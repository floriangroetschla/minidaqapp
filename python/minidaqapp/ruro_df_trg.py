...
def generate_df(network_endpoints,
    ...
    confcmd = mrccmd("conf", "INITIAL", "CONFIGURED",[("ntoq_trigdec", ntoq.Conf(msg_type="dunedaq::dfmessages::TriggerDecision",
    ...
    startcmd = mrccmd("start", "CONFIGURED", "RUNNING", [("qton_token", startpars),
    ...
    stopcmd = mrccmd("stop", "RUNNING", "CONFIGURED", [("ntoq_trigdec", None),
            ("rqg", None),
            ("qton_datareq_.*", None),
            ("ntoq_fragments", None),
            ("ffr", None),
    ...
    resumecmd = mrccmd("resume", "RUNNING", "RUNNING", [("", None)])
...
def generate_ro(network_endpoints,
        NUMBER_OF_DATA_PRODUCERS=2,
        EMULATOR_MODE=False,
        DATA_RATE_SLOWDOWN_FACTOR=1,
        RUN_NUMBER=333, 
    ...
    confcmd = mrccmd("conf", "INITIAL", "CONFIGURED",[("qton_fragments", qton.Conf(msg_type="dunedaq::dataformats::Fragment",
                                           msg_module_name="FragmentNQ",
                                           sender_config=nos.Conf(ipm_plugin_type="ZmqSender",
                                                                  address=network_endpoints["frags"],
                                                                  stype="msgpack"))),
    ...
    startpars = rccmd.StartParams(run=RUN_NUMBER, trigger_interval_ticks=trg_interval_ticks, disable_data_storage=DISABLE_OUTPUT)
    startcmd = mrccmd("start", "CONFIGURED", "RUNNING", [("qton_fragments", startpars),
...
#===============================================================================
def generate_trigemu(network_endpoints,
        NUMBER_OF_DATA_PRODUCERS=2,          
        DATA_RATE_SLOWDOWN_FACTOR=1,
        RUN_NUMBER=333, 
        TRIGGER_RATE_HZ=1.0,
    ...
    # Define modules and queues
    queue_bare_specs = [app.QueueSpec(inst="time_sync_from_netq", kind='FollySPSCQueue', capacity=100),
    ...
    mod_specs = [mspec("qton_trigdec", "QueueToNetwork", [app.QueueInfo(name="input", inst="trigger_decision_to_netq", dir="input")]),

        mspec("ntoq_token", "NetworkToQueue", [app.QueueInfo(name="output", inst="token_from_netq", dir="output")]),

        mspec("ntoq_timesync", "NetworkToQueue", [app.QueueInfo(name="output", inst="time_sync_from_netq", dir="output")]),

        mspec("tde", "TriggerDecisionEmulator", [app.QueueInfo(name="time_sync_source", inst="time_sync_from_netq", dir="input"),
    ...
    confcmd = mrccmd("conf", "INITIAL", "CONFIGURED",[("qton_trigdec", qton.Conf(msg_type="dunedaq::dfmessages::TriggerDecision",
                                           msg_module_name="TriggerDecisionNQ",
                                           sender_config=nos.Conf(ipm_plugin_type="ZmqSender",
                                                                  address=network_endpoints["trigdec"],
                                                                  stype="msgpack"))),
    ...
    startcmd = mrccmd("start", "CONFIGURED", "RUNNING", [("qton_trigdec", startpars),
            ("ntoq_token", startpars),
            ("ntoq_timesync", startpars),
            ("tde", startpars),])

    stopcmd = mrccmd("stop", "RUNNING", "CONFIGURED", [("qton_trigdec", None),
            ("ntoq_timesync", None),
            ("ntoq_token", None),
            ("tde", None),])
    ...
    resumecmd = mrccmd("resume", "RUNNING", "RUNNING", [("tde", tde.ResumeParams(trigger_interval_ticks=trg_interval_ticks))])
...
if __name__ == '__main__':
    ...
    @click.option('--host-ip-ro', default='127.0.0.1')
    @click.argument('json_file_base', type=click.Path(), default='minidaqapp')
    def cli(number_of_data_producers, emulator_mode, data_rate_slowdown_factor, run_number, trigger_rate_hz, token_count, data_file, output_path, disable_data_storage, use_felix, host_ip_df, host_ip_trigemu, host_ip_ro, json_file_base):
        """
        ...
        if use_felix:
            json_file_ro = json_file_base + "-ruflx_ro.json"
        else:
            json_file_ro = json_file_base + "-ruemu_ro.json"

        network_endpoints = {
            "trigdec" : f"tcp://{host_ip_trigemu}:12345",
            "triginh" : f"tcp://{host_ip_df}:12346",
            "timesync": f"tcp://{host_ip_df}:12347",
            "frags": f"tcp://{host_ip_df}:12348"
        }

        port=12349
        for idx in range(number_of_data_producers):
            network_endpoints[f"datareq_{idx}"] = f"tcp://{host_ip_ro}:{port}"
            port = port + 1
        ...
        with open(json_file_trigemu, 'w') as f:
            f.write(generate_trigemu(network_endpoints,
        ...
        with open(json_file_df, 'w') as f:
            f.write(generate_df(network_endpoints,
                    NUMBER_OF_DATA_PRODUCERS = number_of_data_producers,
                    DATA_RATE_SLOWDOWN_FACTOR = data_rate_slowdown_factor,
                    RUN_NUMBER = run_number, 
                    TRIGGER_RATE_HZ = trigger_rate_hz,
        ...
        with open(json_file_ro, 'w') as f:
            f.write(generate_ro(network_endpoints,
                    NUMBER_OF_DATA_PRODUCERS = number_of_data_producers,
                    EMULATOR_MODE = emulator_mode,
                    DATA_RATE_SLOWDOWN_FACTOR = data_rate_slowdown_factor,
                    RUN_NUMBER = run_number, 
